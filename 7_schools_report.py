...
for school in schools:
    if school["NCAA"]["NAIA conference number football (IC2020)"] in conference_schools:
        if (
            ...
            print(f"University Name: {school['instnm']}")
            print(
                f"Total price for in-state sudents that are living off campus (over 50,000): ${school['Total price for in-state students living off campus (not with family)  2020-21 (DRVIC2020)']:,}"
            )
            print()
